...
type KulalaHttpVersion = "HTTP/1.0" | "HTTP/1.1" | "HTTP/2" | "HTTP/3";

type KulalaHttpMethodWithBody = Exclude<
  KulalaHttpMethod,
  "GET" | "HEAD" | "OPTIONS"
>;

type KulalaHttpMethodWithoutBody = Extract<
  KulalaHttpMethod,
  "GET" | "HEAD" | "OPTIONS"
>;

type KulalaHttpScheme = "http" | "https" | "ws" | "wss";
type KulalaHttpURL = `${KulalaHttpScheme}://${string}` | `/${string}`;
...
export type KulalaHttpRequestLineString =
  `${KulalaHttpMethod} ${KulalaHttpURL} ${KulalaHttpVersion}`;
...
type KulalaRequestHeaders = Record<string, string[]>;
...
type KulalaRequestNoBody = {
  method: KulalaHttpMethodWithoutBody;
  url: KulalaHttpURL;
  headers?: KulalaRequestHeaders;
};

type KulalaRequestWithBody = {
  method: KulalaHttpMethodWithBody;
  url: KulalaHttpURL;
  headers?: KulalaRequestHeaders;
  body?: KulalaRequestBody;
};

export type KulalaRequest = KulalaRequestNoBody | KulalaRequestWithBody;
